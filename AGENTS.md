...
- Plan-page pricing controls need their own tags. The generic dialog/main button rules can erase the 5x/20x segmented-control track and disabled `Upgrade to Pro` pill, so tag them with `data-chatskin-plan-toggle`, `data-chatskin-plan-toggle-option`, and `data-chatskin-plan-cta` instead of broadening all dialog button styles.
