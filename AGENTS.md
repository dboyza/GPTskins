...
- Sidebar `Pinned`/`Recents` headers should not get hover pills. Keep section-header controls and icon-only sibling action buttons/wrappers transparent; include `[class*="sidebar"]` in the selector root because the live sidebar may not be a `nav`/`aside`.
